...
for (i in 1:nBS){
IB_conf_matrix <- confusionMatrix(factor(IBpred[[i]]), factor(resp[IBdata[[i]]]))
IBTP <- c(IBTP,IB_conf_matrix$table[2,2])
IBFP <- c(IBFP,IB_conf_matrix$table[1,2])
IBFN <- c(IBFN,IB_conf_matrix$table[2,1])
IBTN <- c(IBTN,IB_conf_matrix$table[1,1])
}
...
MM=matrix(rep(MMM,ncol(pimptable)),nrow=nrow(pimptable),ncol=ncol(pimptable))
 ...
 # pimpsum=  pimpsum/sum(1-as.numeric(unlist(OOBmisclass.list)))
  ...
  ans<-list(AllFits=fitlist, Top5.PI=t5PIs, Predictor.importance=pred.imp, Predictor.frequency=freq.table,
            ...
            pre_X_in.list=pre_X_in.list,OOBmisclass.list=OOBmisclass.list,mean_IBTP=mean(IBTP),mean_IBFP=mean(IBFP),mean_IBTN=mean(IBTN),mean_IBFN=mean(IBFN))
 class(ans)<-"logforest"
 ans
}
####
#train.iwLF <-function(resp, Xs, nBSXVars, anneal.params, nBS=100, maxK=10,dect_order=1)

train.iwLF <-function(resp, Xs, nBS=100, nBSXVars, maxK=10, anneal.params=logreg.anneal.control(start=2, end=-1, iter=50000))
{
h=0.5
err_list=list()
weight_list=list()
...
#vv=train.iwLF(resp=Y, Xs=as.data.frame(X), nBS=100, maxK=10,nBSXVars=floor(ncol(X)^0.7),	 anneal.params=newanneal)

#vv$err_list[[vv$best_k]]


#pred_Y=predict.iwLF(vv,test_X)$LFprediction
#1-sum(abs(pred_Y-test_Y))/length(test_Y)



plot.iwLF<-function(object,TopN){

PI = names(sort(object$fit$PI.importance, decreasing = T)[1:TopN])
score = sort(object$fit$PI.importance, decreasing = T)[1:TopN]

data = data.frame(PI = PI, VIMP.iwLF = score)
data = data %>% arrange(VIMP.iwLF)

p <- ggplot(data, aes(x = reorder(PI, VIMP.iwLF), y = VIMP.iwLF))
p + geom_point() +
geom_segment(aes(x = PI, xend = PI, y = 0, yend = VIMP.iwLF)) +
coord_flip() +
labs(x = "PI", y = "VIMP.iwLF") +
theme_set(theme_bw())}
